...
class LightGBMRanker:
    ...
    def __init__(self, model_path: str = None):
        ...
        if self.lightgbm_available and os.path.exists(self.model_path):
            try:
                logger.info(f"Loading native LightGBM LTR booster from {self.model_path}...")
                self.gbm_model = lgb.Booster(model_file=self.model_path)
                logger.info("LightGBM LTR model loaded successfully.")
            except Exception as e:
                logger.warning(f"Failed to load LightGBM model: {e}. Falling back to dynamic feature weights.")
        else:
            logger.info("Using dynamic feature weights for matching calculations (LightGBM fallback).")
    ...
    def predict_ranking(self, candidate_features: List[Dict[str, float]]) -> List[float]:
        ...
        if self.gbm_model is not None:
            try:
                features_keys = ["experience_years", "skill_match_score", "trajectory_velocity", "exp_ratio", "skill_decay", "semantic_similarity"]
                data = [[row.get(k, 0.0) for k in features_keys] for row in candidate_features]
                predictions = self.gbm_model.predict(data)
                return [float(p) for p in predictions]
            except Exception as e:
                logger.error(f"LightGBM prediction failed: {e}. Falling back to dynamic scoring.")
