...
def calculate_experience_matching(candidate_exp_years: float, jd_required_years: float) -> float:
...
def build_ltr_features(candidate: Dict[str, Any], job: Dict[str, Any], semantic_score: float) -> Dict[str, float]:
    ...
    # Exp extraction
    cand_exp = sum(item.get("duration_months", 0) for item in candidate.get("experience", [])) / 12.0
